...
void BMP::ParallelRowProcessing(std::function<void(int)> processRow, int totalRows) { // accepts a function that will process one row and pass the number of rows
    ...
    std::atomic<int> currentRow(0); // atomic guarantees that two cashiers will not give out the same number.

    auto worker = [&]() {
        while (true) {
            int row = currentRow.fetch_add(1); // gets the current value, increments it by 1, and returns the original value.
            if (row >= totalRows) {
                break;
            }
